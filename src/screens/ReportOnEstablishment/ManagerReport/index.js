...
class ManagerReport extends Component {
      ...
      .then(resp => {
        switch (resp.status) {
          ...
          case (200): {
            ...
            console.log("Own or managequestions==>", resp.data.data)
  ...
  addCheckBoxStatusInArray(questionsArray) {
    ...
    questionsArray.forEach((questionItem, questionIndex) => {
      ...
      else if (questionItem.title === "We test") {
        parentObj = {
          ...questionItem,
  ...
  handleCheckBox(questionIndex, optionIndex) {
    ...
    else if (questionArrayWithStatusTemp[questionIndex].question[optionIndex].option === "Less than 25") {
      questionArrayWithStatusTemp[questionIndex].question.forEach((otionElem, optionIndx) => {
        if (otionElem.option !== "Less than 25") {
          questionArrayWithStatusTemp[questionIndex].question[optionIndx].checkBoxStatus = false
        }
      })
    }
    else if (questionArrayWithStatusTemp[questionIndex].question[optionIndex].option === "25-50") {
      questionArrayWithStatusTemp[questionIndex].question.forEach((otionElem, optionIndx) => {
        if (otionElem.option !== "25-50") {
          questionArrayWithStatusTemp[questionIndex].question[optionIndx].checkBoxStatus = false
        }
      })
    }
    else if (questionArrayWithStatusTemp[questionIndex].question[optionIndex].option === "More than 50") {
      questionArrayWithStatusTemp[questionIndex].question.forEach((otionElem, optionIndx) => {
        if (otionElem.option !== "More than 50") {
          questionArrayWithStatusTemp[questionIndex].question[optionIndx].checkBoxStatus = false
        }
      })
    }
  ...
  render() {
    const { COMMENTS_HERE } = this.props.CommonReport
        ...
        <Container style={{ marginBottom: this.state.footerVisibleToggle ? hp(20.75) : null }}>
          ...
          <AppView style={styles.commentStyle}>
            <AppInput
              ...
              style={{ textAlign: "center", fontSize: normalizeFont(16), marginTop: wp(14) }}
          ...
          {this.state.footerVisibleToggle ? null :
            <AppView style={{ width: wp(50), height: hp(45) }} />}
          <CustomModal
...
const mapStateToProps = state => {
  ...
  return {
    ...
    Manager: state.reducer.language.Manager,
